function [vars_PT,els,locinfo] = locate_points_interp_vars_3d_p...
    (MESH,COMM,gX_PT,vars,OPTS_search,OPTS_interp,els)
% Usage: [vars_PT,els,locinfo] = locate_points_interp_vars_3d_p...
%   (MESH,COMM,gX_PT,vars,OPTS_search,OPTS_interp,els)
%
...
if check_lc
    ...
    lc          = local_coords_3d(GCOORD,EL2NOD{1},els,gX_PT);
...
for img=nmg:-1:1
                                                                            tic
    iel       = img_loc==img;
    lc(:,iel) = local_coords_3d(GCOORD,EL2NOD{img},els(iel),gX_PT(:,iel));
    inot_loc  = any(lc(:,iel)<0-tol_lc) | any(lc(:,iel)>1+tol_lc) | ...
                sum(lc(:,iel))>1+tol_lc;
    if any(inot_loc)
    ...
    lc(:,iel)    = adjust_local_coords( lc(:,iel) ); % *SUBFUNCTION*
...
%     xyz_interp   = interp3d_tet(EL2NOD{img}(1:4,:),els(iel),lc(:,iel),GCOORD_SD');
%     gX_PT(:,iel) = xyz_interp';
    ...
    els(iel)     = calc_tetra_childs(els(iel),lc(:,iel));
    img_loc(iel) = img-1;
...
if nnodel==10 && ~strcmp(OPTS_interp.method_interp,'quadratic')
    ...
    lc2    = local_coords_3d(GCOORD,EL2NOD,els2,gX_PT(:,ind_els));
else
