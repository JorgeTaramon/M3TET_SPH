function [el2sd,topo] = el2sd_bisect_3d_sph(GCOORD,GCOORD_SPH,EL2NOD,COMM,fewer_NB_wght,fid_log)
% Usage: [el2sd,topo] = el2sd_bisect_3d_sph(GCOORD,GCOORD_SPH,EL2NOD,COMM,fewer_NB_wght,fid_log)
%
% Purpose: 
%
% Input:
...
if numlabs==1
    ...
    [el2sd,topo] = find_best_split(nsplit,GCOORD,GCOORD_SPH,EL2NOD',COMM,fewer_NB_wght,fid_log); % *SUBFUNCTION*
else
...
function [el2sd,topo] = find_best_split(nsplit,GCOORD,GCOORD_SPH,EL2NOD,COMM,fewer_NB_wght,fid_log)

ntest = size(nsplit,1);
nnod  = size(unique(EL2NOD),1);
nsd   = COMM.nsd;
