function ASSEMBLY = assembly_stokes_els_in_cone_iso_20_nodes_opt...
...
GCOORD     = [MESH.GCOORD MESH.GCOORD_cubic];  % node coordinates after adding mid-face nodes and edges nodes (1/3 and 2/3)
                                               % coordinates for the isoparametric elements
...
for ib=1:nblk % loop over element blocks
    ...
    % The Jacobian for elements with curved edges needs to be computed at each integration point (cubic 20-node shape to calculate the Jacobian)
    %
    % Rotation matrix:
    % RR_X_90_CCW  = [ 1  0  0 ;
    %                  0  0 -1 ;
    ...
    % ==> ECOORD_y = -ECOORD_z;
    % ==> ECOORD_z =  ECOORD_y;
    ECOORD_x_rot = reshape( GCOORD(1,EL2NOD20(:,il:iu)), size(EL2NOD20,1), nelblk );
    ECOORD_y_rot = reshape(-GCOORD(3,EL2NOD20(:,il:iu)), size(EL2NOD20,1), nelblk );
    ECOORD_z_rot = reshape( GCOORD(2,EL2NOD20(:,il:iu)), size(EL2NOD20,1), nelblk );
    ...
    for ip=1:OPTS.nip
        ...
        %======================================================================================================================
        Jth_SL    = ECOORD_th'*dNUds20{ip};
        Jph_SL    = ECOORD_ph'*dNUds20{ip};
        Jr_SL     = ECOORD_r' *dNUds20{ip};
        ...
        %======================================================================================================================
        th_ip    = ECOORD_th'*NU20{ip};
        ph_ip    = ECOORD_ph'*NU20{ip};
        r_ip     = ECOORD_r' *NU20{ip};
        if any(th_ip == 0)
...
end % END OF FUNCTION assembly_stokes_els_in_cone_iso_20_nodes_opt