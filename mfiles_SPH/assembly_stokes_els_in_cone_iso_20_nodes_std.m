function ASSEMBLY = assembly_stokes_els_in_cone_iso_20_nodes_std...
    (ASSEMBLY,SF,D,B,EL2DOF,VAR,MESH,PHYSICS,NUMSCALE,OPTS)

%==========================================================================
% MODEL PARAMETERS
%==========================================================================
GCOORD          = [MESH.GCOORD MESH.GCOORD_cubic]; % add mid-face coordinates and edges nodes (1/3 and 2/3) for the isoparametric elements
PhaseID         = MESH.PhaseID;
EL2NOD          = double(MESH.EL2NOD{1});
EL2NODP         = double(MESH.EL2NOD{1}(1:4,:));
EL2NOD20        = MESH.EL2NOD_cubic; % connectivity for isoparametric elements with mid-face nodes and edge nodes (1/3 and 2/3)
...
for i = 1:size(els_in_cone_iso,2)
    ...
    % The Jacobian for elements with curved edges needs to be computed at each integration point (cubic 20-node element to calculate the Jacobian)
    GCOORD_this_el         = GCOORD(:,EL2NOD20(:,i));
    ...
    for ip=1:OPTS.nip
        ...
        %       at each integration point, i.e., calculated inside the integration loop. Cubic 20-node shape functions 
        %       are needed to calculate the Jacobian since elements have curved edges in the 90° X-rotated spherical frame. 
        %===========================================================================================================================================================
        ECOORD  = GCOORD_SPH_rot_this_el; % element 20 nodes coordinates
        J_SL    = dNUds20{ip}'*ECOORD';   % Jacobian to transform spherical into local derivatives
...
%                 Fg_el = PHYSICS.g * NUMSCALE.Bscale * (Dens_iel-NUMSCALE.Dens0);
        ...
        %       integration point, i.e., calculated inside the integration loop. Cubic 20-node shape functions are 
        %       needed to calculate the Jacobian since elements have curved edges in the 90° X-rotated spherical frame. 
        %===========================================================================================================================================================
        TH_gp = sum(ECOORD.*repmat(NU20{ip},1,3)',2); % global coordinates (theta,phi,r) of the ip-th integration point
...
%         %---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    %======================================================================
    % Recover the matrices KK, GG and Fb from KK_rot, GG_rot and Fb_rot for those elements that have been rotated 90° around X axis
    ...
    for j = 1:3:ndim*nnodel
        ...
        KKe_temp(2:3:end,j)   =  KKe(3:3:end,j);   % (2,1) is equal to  (3,1)
        KKe_temp(3:3:end,j)   = -KKe(2:3:end,j);   % (3,1) is equal to -(2,1)
        KKe_temp(1:3:end,j+1) =  KKe(1:3:end,j+2); % (1,2) is equal to  (1,3)
        KKe_temp(2:3:end,j+1) =  KKe(3:3:end,j+2); % (2,2) is equal to  (3,3)
        KKe_temp(3:3:end,j+1) = -KKe(2:3:end,j+2); % (3,2) is equal to -(2,3)
        KKe_temp(1:3:end,j+2) = -KKe(1:3:end,j+1); % (1,3) is equal to -(1,2)
        KKe_temp(2:3:end,j+2) = -KKe(3:3:end,j+1); % (2,3) is equal to -(3,2)
        KKe_temp(3:3:end,j+2) =  KKe(2:3:end,j+1); % (3,3) is equal to  (2,2)
    end
    KKe = KKe_temp;
    
    % GG matrix
    GGe_temp            =  GGe;
    GGe_temp(2:3:end,:) =  GGe(3:3:end,:);
    GGe_temp(3:3:end,:) = -GGe(2:3:end,:);
    GGe                 =  GGe_temp;
    
    % Fb vector
    Fbe_temp            =  Fbe;
    Fbe_temp(2:3:end,:) =  Fbe(3:3:end,:);
    Fbe_temp(3:3:end,:) = -Fbe(2:3:end,:);
    Fbe                 =  Fbe_temp;
...
end % END OF FUNCTION assembly_stokes_els_in_cone_iso_20_nodes_std